...
class Utils
{
     ...
     *                                                                     *
     ***********************************************************************/
    public static function send_email(array $user, string $url_activation_page)
    {
        $cod_activation = self::generate_code(5, true);
        // El remitente
        $to = $user["email"];
        // Motivo del correo
        ...
        $msg .= "página: </p>";
        $msg .= "<button><a style='text-decoration:none; color:black;' href='" . $url_activation_page . "'>Activar Cuenta</button>";
        ...
        if($response != false)
        {
            ...
            self::save_log_error("No se ha podido enviar el código de activación.");
        }
    }
}
?>