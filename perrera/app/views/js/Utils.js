...
$('form').on('focusout', function (event) {
    ...
    if (send) {
        $(this).closest('form').find('input[name=edit]').prop('disabled', false).removeClass('bg-secondary').addClass('bg-primary');
    } else {
        $(this).closest('form').find('input[name=edit]').prop('disabled', true).removeClass('bg-primary').addClass('bg-secondary');
    }
