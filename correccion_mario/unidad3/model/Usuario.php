    ...
    public function getUsuarios($conexPDO)
    {
        ...
        if ($conexPDO != null) {
            try {
                ...
                $sentencia = $conexPDO->prepare("SELECT * FROM tiendapablo.usuarios");
        ...
        if (isset($idUsuario) && is_numeric($idUsuario)) {
            ...
            if ($conexPDO != null) {
                try {
                    //Primero introducimos la sentencia a ejecutar con prepare
                    //Ponemos en lugar de valores directamente, interrogaciones
                    $sentencia = $conexPDO->prepare("SELECT * FROM tiendapablo.Usuarios where idUsuario=?");
    ...
    function addUsuario($usuario, $conexPDO)
    {
        ...
        if (isset($usuario) && $conexPDO != null) {
            ...
            try {
                ...
                $sentencia = $conexPDO->prepare("INSERT INTO tiendapablo.Usuarios (nombre, email, edad,sexo,password,salt,activo,codActivacion) VALUES ( :nombre, :email, :edad, :sexo,:password,:salt,:activo,:codActivacion)");
