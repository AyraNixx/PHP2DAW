...
class Login
{
    ...
    const VIEW_INDEX_ADMIN = "../view/index.php";
    ...
    public function login(array $data_login)
    {
        try {
            ...
            if (self::verify_password($data_user["passwd"], $passwd_login, true) === null) {
                // Guardamos el mensaje a mostrar
    ...
    public function change_password(array $data_login)
    {
        ...
        try {
            ...
            if (!$this->verify_password($data_login["passwd"], $data_login[$data_login["passwd_2"]])) {
                // Guardamos mensaje de error
    ...
    public function account_activation(array $data_login)
    {
        ...
        try {
            ...
            // Si el usuario tiene un rol administrativo 
            if ($rol_user == 1) {
                // Incluimos la vista principal del usuario administrador
                require_once(self::VIEW_INDEX_USER);
                // Si el usuario tiene un rol normal (no es administrador)
            } elseif ($rol_user == 2) {
                // Incluimos la vista principal de usuario
                require_once(self::VIEW_INDEX_USER);
            }
        } catch (Exception $e) {
...
if (isset($_REQUEST["action"])) {
    // Guardamos su valor limpiado
    $action = Utils::clean($_REQUEST["action"]);
} else {
    $action = "";
}
...
switch ($action) {
    ...
    case "activation":
        // Comprobamos que estén las claves correo y cod_activacion en el $_REQUEST
        if (!isset($_REQUEST["correo"]) || !isset($_REQUEST["cod_activacion"])) {
            // Cambiamos el valor al atributo msg
        ...
        // Si lo que se quiere es restablecer la contraseña
    case "change_passwd":
    ...
    default:
        // Ponemos $msg a ""
        $msg = "";
        // Usamos header para incluir la vista de Login
        require_once("../view/login.php");
        break;
}
